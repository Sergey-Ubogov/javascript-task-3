...
function searchCrossing(friendBusyTime, currentFreeTime, freeTimes) {
    /*
    В этих функциях весь алгоритм: есть freeTimes - в начале это просто свободное время банка в ПН,
    ВТ и СР, в currentFreeTime находится свободное время в ПН, ВТ или СР. friendBusyTime - это
    время занятости какого-либо из друзей в какой либо день. Представим friendBusyTime как отрезок
    [f1, f2], currentFreeTime как отрезок [c1, c2], тогда, если f1<=c1 и f2>=c1, то есть пересечение слева
    у отрезка [c1, c2] или [c1, c2] содержится в [f1, f2]. Тогда [c1, c2] становится [f2, c2](левая граница
    сдвинулась) или [c1, c1](свободного времени нет в этот день). Если же f1>=c1 и f1 <= c2 то:
    1) если f2 >= c2 значит отрезок [c1, c2] имеет справа пересечение с отрезком [f1, f2] и тогда [c1, c2]
        становится [c1, f1]
    2) если f2<= c2 значит отрезок [f1, f2] содержится в [c1, c2] и тогда [c1, c2] разделяется на 2 отрезка -
        [c1, f1] и [f2, c1] 
    */
